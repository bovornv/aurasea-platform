...
-- STEP 6e — Opportunities (metrics-only; public.today_summary + public.branches)
-- One row per (branch_id, metric_date): best signal when thresholds hit, else type-aware fallback. description = Branch: {name}; sort_score ~58–82.
CREATE OR REPLACE VIEW public.opportunities_today AS
WITH base AS (
    SELECT
