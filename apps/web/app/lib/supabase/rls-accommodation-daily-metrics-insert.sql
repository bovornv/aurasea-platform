...
DROP POLICY IF EXISTS "allow_insert_branch_members" ON fnb_daily_metrics;
DROP POLICY IF EXISTS "allow_select_branch_members" ON fnb_daily_metrics;
DROP POLICY IF EXISTS "allow_update_branch_members" ON fnb_daily_metrics;

CREATE POLICY "allow_insert_branch_members" ON fnb_daily_metrics FOR INSERT TO authenticated
WITH CHECK (EXISTS (SELECT 1 FROM branch_members WHERE branch_members.branch_id = fnb_daily_metrics.branch_id AND branch_members.user_id = auth.uid() AND branch_members.role IN ('owner', 'manager', 'staff')));

CREATE POLICY "allow_select_branch_members" ON fnb_daily_metrics FOR SELECT TO authenticated
USING (EXISTS (SELECT 1 FROM branch_members WHERE branch_members.branch_id = fnb_daily_metrics.branch_id AND branch_members.user_id = auth.uid()));

CREATE POLICY "allow_update_branch_members" ON fnb_daily_metrics FOR UPDATE TO authenticated
USING (EXISTS (SELECT 1 FROM branch_members WHERE branch_members.branch_id = fnb_daily_metrics.branch_id AND branch_members.user_id = auth.uid()))
WITH CHECK (EXISTS (SELECT 1 FROM branch_members WHERE branch_members.branch_id = fnb_daily_metrics.branch_id AND branch_members.user_id = auth.uid() AND branch_members.role IN ('owner', 'manager', 'staff')));
